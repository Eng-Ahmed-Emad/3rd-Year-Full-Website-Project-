...
<head>
    ...
    <title>TRÈS CHIC | <?php echo htmlspecialchars($product['name']); ?></title>
...
<body class="bg-white text-black transition-colors duration-200">
    ...
    <header class="flex justify-between items-center px-6 py-5 border-b transition-colors duration-200">
    ...
    <main class="grid grid-cols-1 md:grid-cols-2 min-h-screen">
        ...
        <div class="p-10 flex flex-col justify-center items-start">
            <div class="w-full max-w-md">
                <h1 class="text-3xl font-semibold mb-2"><?php echo htmlspecialchars($product['name']); ?></h1>
                <p class="text-lg mb-3">€ <?php echo number_format($product['price'], 2, '.', ','); ?></p>
                <p class="text-sm text-gray-600 mb-6">In Stock: <?php echo $product['stock_quantity']; ?> items</p>
                <p class="text-xs text-gray-500 mb-2">Category: <?php echo htmlspecialchars($product['category']); ?></p>
                <p class="text-xs text-gray-500 mb-2">Product ID: <?php echo $product['product_id']; ?></p>
                <p class="text-xs text-gray-500 mb-2">Created at: <?php echo $product['created_at']; ?></p>
                <p class="text-xs text-gray-500 mb-6">Last updated: <?php echo $product['updated_at']; ?></p>
                ...
                <!-- Buttons -->
                <button onclick="addToCartWithQuantity('<?php echo $product['product_id']; ?>', '<?php echo htmlspecialchars($product['name']); ?>', <?php echo $product['price']; ?>, '<?php echo htmlspecialchars($product['image_url']); ?>')" 
                        class="bg-black text-white py-3 rounded-full text-center w-full text-sm font-medium hover:bg-gray-800 transition mb-2">
                    Place in Cart
                </button>
                ...
                <?php if (!empty($product['path'])): ?>
                    <p class="text-xs text-gray-400 mt-2">Path: <?php echo htmlspecialchars($product['path']); ?></p>
                <?php endif; ?>
                ...
                <div class="mt-10 text-sm border-t pt-4">
                    <details>
                        ...
                        <p class="mt-2 text-gray-600">Try this product in store or get personalized advice.</p>
                    </details>
                </div>
            </div>
        </div>

...
// Get stock quantity for suit product
$stmt = $pdo->prepare("SELECT * FROM products WHERE product_id = '5'");
$stmt->execute();
$product = $stmt->fetch(PDO::FETCH_ASSOC);
$stock_quantity = $product ? $product['stock_quantity'] : 0;
$productId = $product ? $product['product_id'] : '5';
$productName = $product ? $product['name'] : 'Costume Élégant';
$productPrice = $product ? $product['price'] : 4250.00;
...
<head>
    ...
    <title>TRÈS CHIC | Costume Élégant</title>
...
<body class="bg-white text-black transition-colors duration-200">
    ...
    <main class="grid grid-cols-1 md:grid-cols-2 min-h-screen">
        ...
        <div class="bg-gray-50 flex justify-center items-center p-8">
            <img src="../Images/Black suit opened.png" alt="Costume Élégant" class="w-full h-auto max-h-[90vh] object-contain" />
        </div>
        ...
        <div class="p-10 flex flex-col justify-center items-start">
            <div class="w-full max-w-md">
                ...
                <!-- Buttons -->
                <button onclick="addToCartWithQuantity('<?php echo $productId; ?>', '<?php echo $productName; ?>', <?php echo $productPrice; ?>, '../Images/Black suit opened.png')" 
                ...
                <div class="text-sm text-gray-700 leading-relaxed">
                    <p class="mb-2">
                        Ce costume noir offre un style intemporel essentiel pour le bureau. Il est confectionné dans un jacquard de laine ton sur ton orné d'un motif LV Blason qui s'étend jusqu'à la doublure.
                    </p>
                ...
                <div class="mt-10 text-sm border-t pt-4">
                    <details class="mb-2">
                        ...
                        <p class="mt-2 text-gray-600">Nettoyage à sec uniquement. Conserver à l'abri de l'humidité et de la lumière directe du soleil.</p>
                    </details>
                    <details>
    ...
    <div id="cartPanel" class="fixed top-0 right-0 h-full w-[400px] bg-white shadow-lg transform translate-x-full transition-transform duration-300 ease-in-out z-50 hidden">
        <div class="p-8 h-full overflow-y-auto">
            ...
            <button onclick="checkout()" class="w-full bg-black text-white py-3 rounded-full text-center text-sm font-medium hover:bg-gray-800 transition mt-4">
                Checkout
            </button>
        </div>
    </div>
